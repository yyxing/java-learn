...
----
    ...
    public class com.devil.stydu.AppConfig { }
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
	@Configuration
	@PropertySource("classpath:application.properties")
	class com.devil.stydu.AppConfig
----
...
The preceding `com.devil.stydu.AppConfig` class is equivalent to the following Spring `<beans/>` XML:
...
----
	public static void main(String[] args) {
		ApplicationContext ctx = new AnnotationConfigApplicationContext(com.devil.stydu.AppConfig.class);
...
----
	...
	fun main() {
		val ctx = AnnotationConfigApplicationContext(com.devil.stydu.AppConfig::class.java)
...
----
	public static void main(String[] args) {
		...
		ctx.register(com.devil.stydu.AppConfig.class, OtherConfig.class);
...
----
	...
	fun main() {
		...
		ctx.register(com.devil.stydu.AppConfig::class.java, OtherConfig::class.java)
...
assuming that `com.devil.stydu.AppConfig` is declared within the `com.acme` package (or any package
...
----
	<web-app>
		...
		<context-param>
			...
			<param-value>com.acme.com.devil.stydu.AppConfig</param-value>
		</context-param>
...
container. In this series examples, we create a `@Configuration` class named `com.devil.stydu.AppConfig` and
...
`@Configuration` annotation and processes the `@Bean` methods declared in `com.devil.stydu.AppConfig`
properly.
...
----
	<beans>
		...
		<bean class="com.acme.com.devil.stydu.AppConfig"/>
...
NOTE: In `system-test-config.xml` file, the `com.devil.stydu.AppConfig` `<bean/>` does not declare an `id`
...
----
	<beans>
		<!-- picks up and registers com.devil.stydu.AppConfig as a bean definition -->
		<context:component-scan base-package="com.acme"/>
		<context:property-placeholder location="classpath:/com/acme/jdbc.properties"/>
